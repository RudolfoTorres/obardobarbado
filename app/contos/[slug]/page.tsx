...
import { Badge, buttonVariants } from "@/components/ui-elements";
...
export default async function TaleDetailPage({ params }: { params: { slug: string } }) {
    const conto = await getContoBySlug(params.slug);
    ...
    const currentIndex = allContos.findIndex(c => c.slug.current === params.slug);
                        ...
                        <div className="prose prose-stone max-w-none">
                            ...
                            {(!conto.content || conto.content.length === 0) && (
                                <p className="font-sans text-ink/70 italic text-xl">
                                    {conto.excerpt}
                                    <br /><br />
                                    [O bardo está afinando sua gaita de fole. O conteúdo completo em breve estará disponível nos pergaminhos da taverna.]
                                </p>
                            )}
                        </div>
